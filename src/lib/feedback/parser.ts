...
export type QueryCommand = 'reading-list' | 'status' | 'stats' | 'weekly' | 'search' | 'trends' | 'digest' | 'recommend';
...
const QUERY_COMMANDS: Set<string> = new Set(['reading-list', 'status', 'stats', 'weekly', 'search', 'trends', 'digest', 'recommend']);
...
export function parseFeedbackMessage(text: string): ParseResult {
  ...
  if (!FEEDBACK_COMMANDS.has(command)) {
    return {
      ...
      message: `Unknown command: /${command}. Supported: /read /skip /save /love /meh /reading-list /status /stats /weekly /search /trends /digest /recommend`,
    };
  }
