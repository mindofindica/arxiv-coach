...
describe('parseFeedbackMessage', () => {
  ...
  it('preserves raw text', () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.feedback.raw).toBe(input.trim());
  });
});
