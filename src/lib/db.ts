...
export function migrate(db: Db) {
  ...
  if (current !== 0) {
    throw new Error(`Unsupported schema migration path: ${current} -> ${SCHEMA_VERSION}`);
  }
  ...
  sqlite.prepare('INSERT OR REPLACE INTO schema_meta (id, version, updated_at) VALUES (1, ?, ?)')
